...
class OrderController extends Controller
{
        ...
        if ($visitor && !empty($visitor->country_code)) {
            ...
            if (count($cart) > 0) {
                for ($i = 0; $i < count($cart); $i++) {
                    ...
                    if (count($product->serials) > 0) {
                        for ($s = 0; $s < $cart[$i]['count']; $s ++) {
                            $oItemSerials = OrderSerial::create([
                                'order_id' => $oItem->id,
                                'serial_id' => $product->serials[$s]->id
                            ]);
                            $product->serials[$s]->sold = 1;
                            $product->serials[$s]->save();
    ...
    public function excute_pay(Request $request){
        ...
        $oItemSerials = OrderSerial::create([
            'order_id' => $oItem->id,
            'serial_id' => $product->serials[0]->id
        ]);
        $product->serials[0]->sold = 1;
        $product->serials[0]->save();
        $product->remaining_quantity = $product->remaining_quantity - 1;
        $product->sold_count = $product->sold_count + 1;
        $product->save();
